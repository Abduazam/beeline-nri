                    ...
                    <form class="js-validation-signin" action="{{ route('login') }}" method="POST">
                        ...
                        <div class="block block-themed block-rounded block-fx-shadow">
                            <div class="block-header bg-gd-dusk">
                                <h3 class="block-title">{{ \App\Models\Widget\TextName::getTextTranslation('sign-in') }}</h3>
                            </div>
                            <div class="block-content">
                                <div class="form-floating mb-4">
                                    ...
                                    <label class="form-label" for="email">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('users', 'email') }}</label>
                                    @error('email')
                                ...
                                <div class="form-floating mb-4">
                                    ...
                                    <label class="form-label" for="password">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('users', 'password') }}</label>
                                    @error('password')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="row">
                                    <div class="col-12 text-sm-end push">
                                        <button type="submit" class="btn btn-lg btn-alt-primary fw-medium w-100">
                                            {{ \App\Models\Widget\TextName::getTextTranslation('sign-in') }}
                                        </button>
                                    </div>
                                </div>
                            </div>
