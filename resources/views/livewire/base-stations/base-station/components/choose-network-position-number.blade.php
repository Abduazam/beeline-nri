<div>
    @if(!is_null($this->position_name))
        <button type="button" class="btn btn-sm" data-bs-toggle="modal" data-bs-target="#modal-choose-position-number{{ $key }}"><u>{{ $this->position_name }}</u></button>
    @else
        <button type="button" class="btn btn-sm btn-alt-secondary" data-bs-toggle="modal" data-bs-target="#modal-choose-position-number{{ $key }}">Выбрать</button>
    @endif

    <div wire:ignore.self class="modal fade" id="modal-choose-position-number{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="modal-choose-position-number{{ $key }}" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="block block-rounded shadow-none mb-0">
                    <form wire:submit.prevent="choose" class="form-border-radius">
                        <div class="block-header block-header-default">
                            <h3 class="block-title text-start">Позиции</h3>
                            ...
                            <div class="table-responsive text-nowrap mb-4">
                                <table class="own-table w-100">
                                    ...
                                    @foreach($positions as $position)
                                        <tr wire:key="position-number-row{{ $position->id }}" wire:click="clickNetworkPosition({{ $position->id }})" class="choose-row @if($position_id === $position->id) bg-success text-white @endif">
