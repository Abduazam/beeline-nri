                ...
                <table class="own-table w-100">
                    ...
                    <tr class="row w-100 h-100 px-0 m-0">
                        <th wire:click="sortBy('id')" class="col-1 d-flex justify-content-center align-items-center cursor-pointer">
                            <span class="pe-2">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('branches', 'id') }}</span>
                            @if($orderBy === 'id')
                        ...
                        <th class="col-7">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('branch_translations', 'name') }}</th>
                        <th wire:click="sortBy('created_at')" class="col-2 d-flex justify-content-center align-items-center cursor-pointer">
                            <span class="pe-2">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('branches', 'created_at') }}</span>
                    ...
                    @foreach($branches as $branch)
                        <tr class="row w-100 h-100 px-0 m-0">
                            ...
                            <td class="col-7 d-flex align-items-center">{{ $branch->translations[0]->name }}</td>
                            <td class="col-2 d-flex justify-content-center align-items-center">{{ $branch->created_at }}</td>
                            <td class="col-2 d-flex justify-content-center align-items-center">
                                @if($this->is_archived == 1)
                                    @can('restore branch')
                                        <livewire:area.branches.restore :branch="$branch" :wire:key="'restore-branch-' . $branch->id"/>
                                    @endcan
                                @else
                                    @can('edit branch')
                                        <livewire:area.branches.edit :branch="$branch" :wire:key="'edit-branch-' . $branch->id"/>
                                    @endcan
                                    @can('delete branch')
                                        <livewire:area.branches.delete :branch="$branch" :wire:key="'delete-branch-' . $branch->id"/>
                                    @endcan
                                @endif
                            </td>
                        </tr>
