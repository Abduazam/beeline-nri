<div>
    ...
    <x-modal.opens.restore target="#modal-restore-region-{{ $this?->region?->id }}" />

    <!-- Restore Branch Modal -->
    <div wire:ignore.self class="modal fade" id="modal-restore-region-{{ $this?->region?->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-restore-region-{{ $this?->region?->id }}s" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="block block-rounded shadow-none mb-0">
                    <form wire:submit.prevent="restore" class="form-border-radius">
                        <x-modal.header action="restore" model="region" />
                                ...
                                <ul class="nav nav-tabs nav-tabs-alt fs-sm" wire:ignore role="tablist">
                                    @foreach($languages as $language)
                                        <li class="nav-item cursor-pointer" role="presentation">
                                            <a class="nav-link @if($loop->index == 0) active @endif" id="btabs-alt-static-restore-{{ $language->slug }}-tab-{{ $this?->region?->id }}" data-bs-toggle="tab" data-bs-target="#btabs-alt-static-restore-{{ $language->slug }}-{{ $this?->region?->id }}" role="tab" aria-controls="btabs-alt-static-restore-{{ $language->slug }}-{{ $this?->region?->id }}" aria-selected="true">{{ $language->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                                <div class="block-content tab-content px-0 fs-sm">
                                    @foreach($languages as $language)
                                        <div wire:ignore.self class="tab-pane @if($loop->index == 0) active @endif" id="btabs-alt-static-restore-{{ $language->slug }}-{{ $this?->region?->id }}" role="tabpanel" aria-labelledby="btabs-alt-static-restore-{{ $language->slug }}-tab-{{ $this?->region?->id }}" tabindex="0">
