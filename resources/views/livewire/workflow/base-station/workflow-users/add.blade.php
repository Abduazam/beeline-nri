<div>
    ...
    <x-modal.opens.add target="#modal-add-user-base-station-workflow-{{ $this->workflow->id }}" icon="fa fa-user-plus" />

    <!-- Add User BaseStation Workflow Modal -->
    <div wire:ignore.self
         class="modal fade"
         id="modal-add-user-base-station-workflow-{{ $this->workflow->id }}"
         tabindex="-1"
         role="dialog"
         aria-labelledby="modal-add-user-base-station-workflow-{{ $this->workflow->id }}"
         aria-hidden="true">
                ...
                <div class="block block-rounded shadow-none mb-0">
                    <form wire:submit.prevent="add" class="form-border-radius">
                        ...
                        <div class="block-content fs-sm text-start">
                            <div class="mb-4">
                                <label class="form-label" for="users">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('base_station_workflows', 'title') }}</label>
    ...
    <script>
        $(document).ready(function () {
            $('#users-' + {{ $this->workflow->id }}).select2({
                dropdownParent: $("#modal-add-user-base-station-workflow-" + {{ $this->workflow->id }})
            }).on('change', function () {
