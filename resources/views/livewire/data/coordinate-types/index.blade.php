            ...
            <div class="filter-table pb-4">
                <div class="row w-100 h-100 m-0 p-0">
                    <div class="col-sm-4 p-0">
                        <x-filter.search placeholder="Search coordinate"/>
                    </div>
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-5 text-end pe-0">
                        @can('create coordinate type')
                            <livewire:data.coordinate-types.create />
                        @endcan
                    </div>
                </div>
            </div>
                ...
                <table class="own-table w-100">
                    ...
                    <tr class="row w-100 h-100 px-0 m-0">
                        <th wire:click="sortBy('id')" class="col-1 d-flex justify-content-center align-items-center cursor-pointer">
                            <span class="pe-2">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('coordinate_types', 'id') }}</span>
                            @if($orderBy === 'id')
                        ...
                        <th class="col-7">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('coordinate_types', 'name') }}</th>
                        <th wire:click="sortBy('created_at')" class="col-2 d-flex justify-content-center align-items-center cursor-pointer">
                            <span class="pe-2">{{ \App\Models\Widget\TableColumnName::getColumnTranslation('coordinate_types', 'created_at') }}</span>
                    ...
                    @foreach($types as $type)
                        <tr class="row w-100 h-100 px-0 m-0">
                            ...
                            <td class="col-7 d-flex align-items-center">{{ $type->name }}</td>
                            ...
                            <td class="col-2 d-flex justify-content-center align-items-center">
                                @if($this->is_archived == 1)
                                    @can('restore coordinate type')
                                        <livewire:data.coordinate-types.restore :type="$type" :wire:key="'restore-coordinate-type-' . $type->id"/>
                                    @endcan
                                @else
                                    @can('edit coordinate type')
                                        <livewire:data.coordinate-types.edit :type="$type" :wire:key="'edit-coordinate-type-' . $type->id"/>
                                    @endcan
                                    @can('delete coordinate type')
                                        <livewire:data.coordinate-types.delete :type="$type" :wire:key="'delete-coordinate-type-' . $type->id"/>
                                    @endcan
                                @endif
                            </td>
                        </tr>
